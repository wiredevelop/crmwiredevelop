    ...
    <BaseLayout>
        ...
        <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between mb-6">
            <div>
                ...
                <p class="text-sm text-gray-500" v-if="client.company">
            ...
            <div class="flex flex-wrap gap-2">
                <Link :href="`/clients/${client.id}/edit`"
                      class="px-3 py-1 bg-blue-600 text-white rounded">Editar</Link>

                <Link as="button" method="post"
                      :href="`/clients/${client.id}/duplicate`"
                      class="px-3 py-1 bg-yellow-500 text-white rounded">Duplicar</Link>

                <Link :href="`/projects/create?client=${client.id}`"
                      class="px-3 py-1 bg-green-600 text-white rounded">Novo Projeto</Link>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-8">
            ...
            <div class="bg-white shadow rounded p-6 xl:col-span-2">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    ...
                    <div class="sm:col-span-2">
                        <strong>Notas (formulário):</strong>
            ...
            <div class="bg-white shadow rounded p-6">
                ...
                <div class="flex flex-col gap-2 sm:flex-row mb-4">
                    <input v-model="noteForm.note"
                           placeholder="Adicionar nota..."
                           class="w-full border rounded px-3 py-2" />

                    <button @click="addNote"
                            class="bg-[#015557] text-white px-4 py-2 rounded">
                        Guardar
                    </button>
                </div>
            ...
            <form @submit.prevent="addCredential" class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div>
                    ...
                    <select v-model="credentialForm.object_id" class="input" required :disabled="!credentialObjects.length">
                        ...
                        <option v-for="object in credentialObjects" :key="object.id" :value="object.id">
                            {{ object.name }}
                        </option>
                    </select>
                </div>
                ...
                <div class="md:col-span-2 flex justify-end">
                    <button type="submit" class="bg-[#015557] text-white px-4 py-2 rounded" :disabled="!credentialObjects.length">
                        Guardar Senha
                    </button>
                </div>
            </form>
                    ...
                    <summary class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between p-4 cursor-pointer">
                        <div>
                            ...
                            <p class="text-xs text-gray-400">
                                {{ object.credentials?.length || 0 }} senha(s)
                            </p>
                        </div>
                        <div class="flex items-center gap-3 text-xs">
                            <a
                                :href="`/clients/${client.id}/credential-objects/${object.id}/export`"
                                class="text-[#015557] hover:underline"
                                @click.stop
                            >
                                Exportar
                            </a>
                            <Link
                                as="button"
                                method="delete"
                                :href="`/clients/${client.id}/credential-objects/${object.id}`"
                                ...
                                <tbody>
                                    <tr v-for="cred in object.credentials" :key="cred.id" class="border-b hover:bg-gray-50">
                                        ...
                                        <td class="py-2 px-2 text-right">
                                            <Link as="button" method="delete"
                                                  :href="`/clients/${client.id}/credentials/${cred.id}`"
        ...
        <div class="bg-white shadow rounded p-6 mb-6">
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-4">
                <h2 class="text-xl font-semibold">Últimos Projetos</h2>
                <Link href="/projects" class="text-sm text-[#015557] hover:underline">
                    Ver todos
                </Link>
            </div>
        ...
        <div class="bg-white shadow rounded p-6 mb-6">
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-4">
                <h2 class="text-xl font-semibold">Últimas Faturas</h2>
                <Link href="/invoices" class="text-sm text-[#015557] hover:underline">
                    Ver todas
                </Link>
            </div>
            ...
            <div v-if="invoices.length" class="overflow-x-auto">
                ...
                <tr v-for="i in invoices" :key="i.id" class="border-b hover:bg-gray-50">
                    ...
                    <td class="py-2 px-2 text-right">
                        <Link :href="`/invoices/${i.id}/pdf`"
                              class="text-blue-600 hover:underline text-xs" target="_blank">
                            PDF
                        </Link>
                    </td>
                </tr>
            ...
            <p v-else class="text-gray-500 text-sm">Sem faturas registadas.</p>
        </div>
    </BaseLayout>
</template>
