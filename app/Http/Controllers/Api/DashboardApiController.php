...
class DashboardApiController extends Controller
{
    ...
    public function index(): JsonResponse
    {
        ...
        $stats = [
            ...
            'paid_this_month' => (clone $invoicesQuery)->whereMonth('paid_at', now()->month)->whereYear('paid_at', now()->year)->sum('total'),
        ...
        if (! $isClientUser && $salesGoal && $salesGoal > 0) {
            ...
            $paidInvoicesDetails = (clone $paidInvoicesQuery)
                ->select(['invoices.id', 'invoices.number', 'invoices.total', 'invoices.project_id', 'projects.name as project_name', 'quotes.price_development as project_development'])
                ...
                ->map(fn ($invoice) => [
                    ...
                    'amount' => $invoice->project_id ? (float) ($invoice->project_development ?? 0) : (float) ($invoice->total ?? 0),
                ])
                ->toArray();
                ...
                ->whereHas('project', function ($query) {
                    $query->where(function ($subQuery) {
                        $subQuery->whereDoesntHave('invoice')
                            ->orWhereHas('invoice', fn ($invoiceQuery) => $invoiceQuery->where('status', '!=', 'pago'));
                    });
                });

            $installmentsThisYear = (float) (clone $installmentsQuery)->selectRaw('SUM(COALESCE(amount, 0)) as total')->value('total');
            ...
            $salesBreakdownDetails = [
                ...
                'installments' => $installmentsQuery->with(['project:id,name', 'client:id,name'])->orderByDesc('paid_at')->get()->map(fn ($installment) => [
                    'id' => $installment->id,
                ...
                'adjudications' => $adjudicationsQuery
                    ->select(['quotes.id', 'quotes.project_id', 'quotes.price_development', 'quotes.adjudication_percent', 'quotes.adjudication_paid_at', 'projects.name as project_name'])
        ...
        return $this->success([
            ...
            'sales_breakdown_details' => $salesBreakdownDetails,
            'sales' => $isClientUser ? $this->clientSales() : [],
            'installments' => $isClientUser ? $this->clientInstallments() : [],
            'registered_invoices' => $isClientUser
                ? $this->scopeByClient(Invoice::with(['client', 'project']))->orderByDesc('issued_at')->get()
                : [],
            'recent_clients' => $this->scopeClients(Client::query())->latest()->take(5)->get(),
            'recent_projects' => $this->scopeByClient(Project::with('client')->where('is_hidden', false))->latest()->take(5)->get(),
            'recent_invoices' => $this->scopeByClient(Invoice::with(['client', 'project']))->latest()->take(5)->get(),
            'pending_invoices' => $this->scopeByClient(Invoice::with('client'))->where('status', 'pendente')->orderBy('due_at')->take(5)->get(),
        ]);
    }
    ...
    private function clientSales(): array
    {
        ...
        return $projectSales->merge($productSales)->sortByDesc('sort_at')->values()->map(fn ($item) => Arr::except($item, ['sort_at']))->toArray();
    }
