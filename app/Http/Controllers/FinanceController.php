...
class FinanceController extends Controller
{
    ...
    public function index(): Response
    {
        ...
        $installments = $this->scopeByClient(Installment::with(['project:id,name', 'client:id,name', 'invoice:id,number']))
            ->orderByDesc('paid_at')
            ->take(200)
            ->get()
            ->map(fn ($installment) => [
        ...
        $invoices = $this->scopeByClient(Invoice::with(['project:id,name', 'client:id,name']))
    ...
    public function storeInstallment(Request $request): RedirectResponse
    {
        ...
        return back()->with('success', 'Parcela registada.');
    }

    public function destroyInstallment(Installment $installment): RedirectResponse
    {
        $installment->delete();
    ...
    public function updateInstallment(Request $request, string $type, int $id): RedirectResponse
    {
        $data = $request->validate([
        ...
        if ($data['is_installment']) {
            ...
            if ($count < 2) {
                return back()->withErrors([
                    'installment_count' => 'Indica o numero de parcelas.',
                ]);
            }
        }
    ...
    public function updateToInvoice(Request $request, int $id): RedirectResponse
    {
        $data = $request->validate([
    ...
    public function updateProjectToInvoice(Request $request, int $id): RedirectResponse
    {
        $data = $request->validate([
            'to_invoice' => ['required', 'boolean'],
        ]);
    ...
    public function bulkUninvoice(Request $request): RedirectResponse
    {
        $data = $request->validate([
        ...
        $projects = Project::with(['invoice', 'invoice.items'])
        ...
        return back()->with('success', 'Faturacao removida.');
    }

    public function bulkToInvoice(Request $request): RedirectResponse
    {
        $data = $request->validate([
            'items' => ['required', 'array', 'min:1'],
            'items.*.source' => ['required', 'in:project,transaction'],
        ...
        $projects = Project::with(['client', 'quote', 'invoice', 'invoice.items'])
            ->whereIn('id', $projectIds)
            ->get();
        ...
        if ($clientIds->count() > 1) {
            return back()->withErrors([
                'bulk' => 'Seleciona vendas do mesmo cliente.',
            ]);
        }

        if ($projects->count() > 1) {
            return back()->withErrors([
                'bulk' => 'Seleciona apenas um projeto de cada vez.',
            ]);
        }
        ...
        return back()->with('success', 'Faturas criadas.');
    }
    ...
    private function addTransactionToInvoice(Invoice $invoice, WalletTransaction $transaction): void
    {
        ...
        $invoice->items()->create([
            'description' => $transaction->description
                ?? ($transaction->product?->name ?? 'Venda de pack/produto'),
            'quantity' => 1,
            'unit_price' => $total,
            'total' => $total,
