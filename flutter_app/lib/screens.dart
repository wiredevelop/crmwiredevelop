...
class _ClientWalletScreenState extends State<ClientWalletScreen> {
  ...
  String _stripeMethodLabel(String method) {
    switch (method) {
      case 'card':
        return 'Cartão';
      case 'link':
    ...
    final completed = await Navigator.of(context).push<bool>(
      CupertinoPageRoute(
        builder: (context) => ClientCheckoutFlowScreen(
          ...
          stripeMethodLabel: _stripeMethodLabel,
        ),
      ),
    );
...
class ClientCheckoutFlowScreen extends StatefulWidget {
  const ClientCheckoutFlowScreen({
    ...
    required this.stripeMethodLabel,
  });
  ...
  final String Function(String method) stripeMethodLabel;
...
class _ClientCheckoutFlowScreenState extends State<ClientCheckoutFlowScreen> {
  ...
  Future<void> _prepareCheckout() async {
    try {
      ...
      setState(() {
        _checkout = data;
        _availableMethods =
            ((data['available_payment_methods'] as List?) ?? const [])
                .map((item) => item.toString())
                .toList();
      });
    } on ApiException catch (error) {
  ...
  String get _paymentIntentId =>
      _checkout?['payment_intent_id']?.toString() ?? '';

  BillingDetails _billingDetails() {
    return BillingDetails(
      name: widget.clientName,
      email: widget.clientEmail == '—' ? null : widget.clientEmail,
      phone: widget.clientPhone == '—' ? null : widget.clientPhone,
    );
  }

  Future<void> _cancelCheckout() async {
    final paymentIntentId = _paymentIntentId;
    if (_cancelled || paymentIntentId.isEmpty) {
      return;
    }
  ...
  Future<void> _pay() async {
    ...
    if (checkout == null || _paying) {
      return;
    }

    final publishableKey = checkout['publishable_key']?.toString() ?? '';
    final paymentIntentId = _paymentIntentId;
    if (publishableKey.isEmpty || paymentIntentId.isEmpty) {
      await showMessage(
        context,
        title: 'Erro',
      ...
      await Stripe.instance.initPaymentSheet(
        paymentSheetParameters: SetupPaymentSheetParameters(
          ...
          paymentIntentClientSecret:
              checkout['payment_intent_client_secret']?.toString() ?? '',
          customerId: checkout['customer_id']?.toString(),
          customerEphemeralKeySecret: checkout['customer_ephemeral_key_secret']
              ?.toString(),
          billingDetails: _billingDetails(),
                    ...
                    children: [
                      ...
                      CardSection(
                        ...
                        child: Column(
                          ...
                          children: [
                            const Text(
                              'Métodos Stripe disponíveis neste pagamento. A seleção final é feita dentro do checkout Stripe, sem redireções externas.',
                            ),
                            const SizedBox(height: 10),
                            if (_availableMethods.isEmpty)
                              const Text('Sem métodos disponíveis.')
                            else
                              Wrap(
                                spacing: 8,
                                runSpacing: 8,
                                  ...
                                  for (final method in _availableMethods)
                                    Container(
                                      ...
                                      decoration: BoxDecoration(
                                        ...
                                        border: Border.all(
                                          color: const Color(0x260E4D50),
                                        ),
                                      ),
                                      child: Text(
                                        widget.stripeMethodLabel(method),
                                        style: const TextStyle(
                                          fontWeight: FontWeight.w700,
                                        ),
                                      ),
